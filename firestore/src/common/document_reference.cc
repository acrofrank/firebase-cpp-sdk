...
Future<DocumentSnapshot> DocumentReference::Get(Source source) const {
  UnityIssue1154TestLog("DocumentReference::Get() start");
  if (!internal_) {
    UnityIssue1154TestLog("DocumentReference::Get() done (!internal_)");
    return FailedFuture<DocumentSnapshot>();
  }
  auto result = internal_->Get(source);
  UnityIssue1154TestLog("DocumentReference::Get() done");
  return result;
}

Future<void> DocumentReference::Set(const MapFieldValue& data,
                                    const SetOptions& options) {
  UnityIssue1154TestLog("DocumentReference::Set() start");
  if (!internal_) {
    UnityIssue1154TestLog("DocumentReference::Set() done (!internal_)");
    return FailedFuture<void>();
  }
  auto result = internal_->Set(data, options);
  UnityIssue1154TestLog("DocumentReference::Set() done");
  return result;
}
